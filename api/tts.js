...
const ALLOWED_LANGUAGES = new Set([
  'uz', 'ja', 'en', 'ru', 'ko', 'zh', 'es', 'fr', 'de', 'tr', 'ar', 'it', 'pt', 'hi', 'id', 'vi', 'th'
]);
...
export default async function handler(req) {
  ...
  if (req.method !== 'GET' && req.method !== 'POST' && req.method !== 'HEAD') {
    return new Response(JSON.stringify({ error: 'Method Not Allowed', message: 'Faqat GET yoki POST so\'rovlari qo\'llab-quvvatlanadi.' }), {
      status: 405,
      headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
    });
  }

  // 2. Authentication: strictly verify Supabase JWT
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      return new Response(JSON.stringify({ error: 'Bad Request', message: 'Yaroqsiz JSON formati.' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
      });
    }
    ...
    if (!body || typeof body !== 'object' || Array.isArray(body)) {
      return new Response(JSON.stringify({ error: 'Bad Request', message: 'Request body ob\'ekt bo\'lishi shart.' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
      });
    }
  ...
  if (typeof rawText !== 'string') {
    return new Response(JSON.stringify({ error: 'Bad Request', message: 'Matn (text) maydoni satr (string) bo\'lishi shart.' }), {
      status: 400,
      headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
    });
  }
  ...
  if (rawText.length > 200) {
    return new Response(JSON.stringify({ error: 'Bad Request', message: 'Matn uzunligi maksimal 200 belgidan oshmasligi kerak.' }), {
      status: 400,
      headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
    });
  }
  ...
  if (cleanText.length === 0) {
    return new Response(JSON.stringify({ error: 'Bad Request', message: 'Bo\'sh matnni ovozlashtirib bo\'lmaydi.' }), {
      status: 400,
      headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
    });
  }

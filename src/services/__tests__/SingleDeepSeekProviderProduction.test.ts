...
describe('Single DeepSeek Provider Production Architecture Test Suite', () => {
  ...
  it('8. AI failure does not generate a fake fallback plan in generateWeeklyPlan', async () => {
    ...
    await expect(
      PersonalLearningPlanEngine.generateWeeklyPlan('user-fail-test', sampleGoal, 1),
  ...
  it('9. AI failure returns AI_UNAVAILABLE status message when response is empty/invalid', async () => {
    ...
    await expect(
      PersonalLearningPlanEngine.generateWeeklyPlan('user-unavail-test', sampleGoal, 1),
  ...
  it('15. DeepSeek response schema is strictly validated by parseAndValidateWeeklyPlan', () => {
    ...
    expect(
      PersonalLearningPlanEngine.parseAndValidateWeeklyPlan(
        'not valid json {',
        goal,
        1,
        'user-schema-test',
    ...
    expect(
      PersonalLearningPlanEngine.parseAndValidateWeeklyPlan('[]', goal, 1, 'user-schema-test'),
    ).toBeNull();
    ...
    const validated = PersonalLearningPlanEngine.parseAndValidateWeeklyPlan(
      validJson,
      goal,
      1,
      'user-schema-test',

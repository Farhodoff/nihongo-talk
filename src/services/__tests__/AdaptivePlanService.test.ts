...
describe('AdaptivePlanService Unit Tests', () => {
  ...
  it('12. should include advanced IELTS prep for B2/C1 English learners', () => {
    ...
    const plan = AdaptivePlanService.generateDailyPlan(state, 60);
    const hasEnrichment = plan.items.some(
      (i) => i.type === 'speaking' || i.type === 'writing' || i.type === 'reading',
    );
  ...
  it('13. should include advanced JLPT practice for N3/N2 Japanese learners', () => {
    ...
    const plan = AdaptivePlanService.generateDailyPlan(state, 60);
    const hasJaEnrichment = plan.items.some(
      (i) => i.route.includes('jlpt') || i.route.includes('speaking'),
    );
  ...
  it('21. should route to /study-mode?lang=ja when primaryLanguage is ja', () => {
    const state = createBaseState({
      primaryLanguage: 'ja',
      reviewSummary: {
        totalCards: 32,
    ...
    expect(srsItem).toBeDefined();
    expect(srsItem?.route).toBe('/study-mode?lang=ja');
  });
});

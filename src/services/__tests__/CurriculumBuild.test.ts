...
describe('Phase 17 — Curriculum Quality & Progression Verification', () => {
  ...
  it('7. completed lesson filtering', () => {
    localStorage.setItem(
      'study_planner_learning_signals_test-user',
      JSON.stringify([
        {
          id: 'sig-test-1',
          userId: 'test-user',
  ...
  it('8. weak skill priority', async () => {
    localStorage.setItem(
      'study_planner_mastery_evidence_test-user_en',
      JSON.stringify([
  ...
  it('12. IELTS mapping exists', () => {
    ...
    const ieltsLessons = b2Level?.units
      .flatMap((u) => u.lessons)
      .filter((l) => l.examTrack === 'IELTS');
  ...
  it('20. fallback level filtering matches current level', () => {
    ...
    vi.spyOn(PersonalLearningPlanService, 'getCompletedLessonIds').mockReturnValue(['en-a1-u1-l1']);
    const plan = PersonalLearningPlanEngine.parseAndValidateWeeklyPlan(
      JSON.stringify({
        days: ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'].map(
          (d) => ({
      ...
      sampleGoalEnB2 as any,
      1,
      'test-user',
    )!;
    const allTasks = plan.days.flatMap((d) => d.tasks);
    allTasks.forEach((task) => {
      if (task.contentId && task.contentId.startsWith('en-') && task.type === 'lesson') {

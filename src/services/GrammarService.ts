...
export const GrammarService = {
  ...
  async fetchLessons(language: string = 'en'): Promise<IeltsGrammarTopic[]> {
    try {
      ...
      if (error) {
        console.warn(
          'GrammarService.fetchLessons warning (falling back to built-in seed):',
          error.message,
        );
  ...
  async getLessonBySlug(slug: string): Promise<IeltsGrammarTopic | null> {
    try {
      ...
      if (error || !data) {
        ...
        const fallback = fallbackData.find((t: IeltsGrammarTopic) => t.id === slug);
   ...
   */
  async createLesson(
    topic: Partial<IeltsGrammarTopic> & { slug?: string },
  ): Promise<IeltsGrammarTopic | null> {
    const slug =
      topic.slug ||
      topic.id ||
    ...
    const { data, error } = await supabase.from('grammar_lessons').insert([row]).select().single();
  ...
  async updateLesson(
    idOrSlug: string,
    updates: Partial<IeltsGrammarTopic>,
  ): Promise<IeltsGrammarTopic | null> {
    const payload: any = {

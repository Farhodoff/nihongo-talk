...
export const JAPANESE_SKILLS: MasterySkill[] = [
  'kanji',
  'grammar',
  'vocabulary',
  'reading',
...
export const AdaptiveQuestionEngine = {
  ...
  initializeSession(
    ...
    const startDifficulty: 'easy' | 'medium' | 'hard' =
      startLevel === 'A1' || startLevel === 'N5' ? 'easy' : 'medium';
  ...
  selectNextQuestion(
    ...
    const exact = available.find(
      (q) =>
        q.level === targetLevel && q.skill === targetSkill && q.difficulty === targetDifficulty,
    );
    if (exact) return exact;

    // 2. Match: level + skill (any difficulty)
    const matchLevelSkill = available.find(
      (q) => q.level === targetLevel && q.skill === targetSkill,
    );
    if (matchLevelSkill) return matchLevelSkill;

    // 3. Match: level + difficulty (any skill)
    const matchLevelDiff = available.find(
      (q) => q.level === targetLevel && q.difficulty === targetDifficulty,
    );
  ...
  evaluateAdaptiveSession(
    ...
    for (const sk of skills) {
      ...
      if (totalQuestions < 2) {
        ...
        status = 'strong';
        reason = isJa
          ? "Ushbu ko'nikma bo'yicha kuchli bilim namoyon etildi."
          : 'Demonstrated high competence in this skill area.';
        ...
        status = 'weak';
        reason = isJa
          ? "Ko'nikma ustida ko'proq ishlash tavsiya etiladi."
          : 'Identifies as a potential focus area for study.';
    ...
    if (isJa) {
      if (recommendedStartLevel === 'N1' || recommendedStartLevel === 'N2')
        firstLessonId = 'ja-n3-u1-l1'; // Cap to sample content availability
      else if (recommendedStartLevel === 'N3') firstLessonId = 'ja-n3-u1-l1';
      else if (recommendedStartLevel === 'N4') firstLessonId = 'ja-minna-l26';
      else firstLessonId = 'ja-n5-u1-l1';
    } else {
      if (recommendedStartLevel === 'C1' || recommendedStartLevel === 'C2')
    ...
    const overallScore =
      totalAnswersCount > 0 ? Math.round((totalCorrectAnswers / totalAnswersCount) * 100) : 0;

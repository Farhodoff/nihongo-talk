...
export class ErrorVaultService {
  ...
  public static getErrors(userId?: string | null): SpeakingErrorItem[] {
    ...
    return safeLocalStorage.getJSON<SpeakingErrorItem[]>(this.getStorageKey(userId), []);
  }
  ...
  public static async syncFromDB(explicitUserId?: string | null): Promise<SpeakingErrorItem[]> {
    try {
      ...
      if (!error && dbRows && dbRows.length > 0) {
        ...
        const merged = [...mappedFromDB, ...localErrors.filter((e) => !dbIds.has(e.id))].slice(
          0,
          100,
        );
        safeLocalStorage.setJSON(userKey, merged);
        return merged;
      }
    } catch (e) {
      logger.warn('ErrorVaultService', 'Failed to sync error vault from DB', e);
    }
  ...
  public static logErrors(
    errors: Omit<SpeakingErrorItem, 'id' | 'timestamp' | 'timesReviewed'>[],
    ...
    try {
      safeLocalStorage.setJSON(userKey, updated);
    } catch (e) {
      logger.error('ErrorVaultService', 'Error saving to error vault', e);
    }
    ...
    const syncToDb = (activeId: string) => {
      if (activeId && activeId !== 'guest') {
        ...
        supabase
          ...
          .upsert(payloads)
          .then(({ error }) => {
            if (error) logger.warn('ErrorVaultService', 'DB upsert error', error);
          });
      }
    };
   ...
   */
  public static convertErrorsToFlashcards(
    language: 'en' | 'ja' = 'en',
    subjectId?: string,
    userId?: string | null,
  ): Partial<Flashcard>[] {
   ...
   */
  public static getWeakItemsPromptSnippet(
    language: 'en' | 'ja' = 'en',
    userId?: string | null,
  ): string {
    const errors = this.getErrors(userId)
      .filter((e) => e.language === language)
      .slice(0, 5);
    if (errors.length === 0) return '';

    const itemsStr = errors
      .map(
        (e) =>
          `- User previously said "${e.verbatim}" instead of "${e.correction}" (${e.explanation})`,
      )
      .join('\n');
  ...
  public static clearVault(userId?: string | null): void {
    try {
      ...
      logger.error('ErrorVaultService', 'Error clearing vault', e);
    }
  }
}

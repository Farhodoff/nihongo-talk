...
export const RoadmapService = {
  ...
  getLearningRoadmap(state: UserLearningState): LearningRoadmap {
    ...
    const currentLevelNode =
      course.levels.find((l) => l.code.toLowerCase() === currentLevelCode.toLowerCase()) ||
      course.levels[0];
    ...
    const roadmapLevels: RoadmapLevelNode[] = course.levels.map((level) => {
      ...
      const roadmapUnits: RoadmapUnitNode[] = level.units.map((unit) => {
        ...
        const roadmapLessons: RoadmapLessonNode[] = unit.lessons.map((lesson) => {
          ...
          if (isPast) {
            ...
            // Current Level logic
            const completedIds = PersonalLearningPlanService.getCompletedLessonIds(
              state.userId,
            ...
            const isDone =
              ...
              (prog ? prog.isCompleted || prog.completed || false : false);
              ...
              if (prereqIds.length > 0) {
                for (const prereqId of prereqIds) {
                  ...
                  const pDone =
                    ...
                    (pProg ? pProg.isCompleted || pProg.completed || false : false);
                  if (!pDone) {
                    prereqsMet = false;
                    lockReason = isJa
            ...
            const isWeak = state.masteryProfile?.topWeaknesses.some(
              (w) => w.skill === lesson.skill,
            );
        ...
        const unitStatus: NodeStatus = isPast
          ? 'completed'
          : isFuture
            ? 'locked'
            : unitProg === 100
      ...
      const levelProg =
        levelLessonsCount > 0 ? Math.round((levelCompletedCount / levelLessonsCount) * 100) : 0;

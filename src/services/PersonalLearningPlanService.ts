...
export const PersonalLearningPlanService = {
  ...
  getActiveGoal(userId: string = 'guest'): PersonalLearningGoal | null {
    ...
    return safeLocalStorage.getJSON<PersonalLearningGoal | null>(key, null);
  },
  ...
  async saveGoal(userId: string = 'guest', goal: PersonalLearningGoal): Promise<void> {
    ...
    safeLocalStorage.setJSON(key, goal);
    ...
    if (userId && userId !== 'guest') {
      ...
      if (data) {
        ...
        safeLocalStorage.setJSON(key, updatedGoal);
      }
    }
  },
  ...
  async fetchActiveGoalFromServer(userId: string): Promise<PersonalLearningGoal | null> {
    ...
    try {
      ...
      if (data) {
        ...
        safeLocalStorage.setJSON(`${GOAL_STORAGE_PREFIX}${userId}`, goal);
        return goal;
      } else {
  ...
  getWeeklyPlans(userId: string = 'guest'): WeeklyLearningPlan[] {
    const allPlans = safeLocalStorage.getJSON<WeeklyLearningPlan[]>(PLANS_STORAGE_KEY, []);
    return allPlans.filter((p: WeeklyLearningPlan) => p.userId === userId);
  },
  ...
  getLatestWeeklyPlan(userId: string = 'guest', goalId: string): WeeklyLearningPlan | null {
    ...
    let plans = allUserPlans.filter(
      (p) => toDeterministicUUID(p.goalId) === dbGoalId || p.goalId === goalId,
    );
  ...
  async fetchWeeklyPlansFromServer(userId: string): Promise<WeeklyLearningPlan[]> {
    ...
    try {
      ...
      const otherUsersPlans = safeLocalStorage
        .getJSON<WeeklyLearningPlan[]>(PLANS_STORAGE_KEY, [])
        .filter((p) => p.userId !== userId);
      const merged = [...otherUsersPlans, ...plans];
      safeLocalStorage.setJSON(PLANS_STORAGE_KEY, merged);
    ...
    if (existingIndex !== -1) {
      ...
      const updated = allPlans.map((p) => {
        if (p.goalId === plan.goalId && p.weekNumber === plan.weekNumber && p.status === 'active') {
          return { ...p, status: 'archived' as PlanStatus };
        }
        return p;
      });
      updated.push(plan);
      safeLocalStorage.setJSON(PLANS_STORAGE_KEY, updated);
    }
    if (existingIndex !== -1) {
      safeLocalStorage.setJSON(PLANS_STORAGE_KEY, allPlans);
    }
  ...
  async completePlanTask(
    ...
    planId: string,
    taskId: string,
  ): Promise<WeeklyLearningPlan | null> {
  ...
  async fetchWeeklyEvaluationsFromServer(userId: string): Promise<WeeklyEvaluation[]> {
    ...
    try {
      ...
      // Sync cache (preserve other users' data)
      const otherUsersEvals = safeLocalStorage
        .getJSON<WeeklyEvaluation[]>(EVALS_STORAGE_KEY, [])
        .filter((ev) => ev.userId !== userId);
      const merged = [...otherUsersEvals, ...evals];
      safeLocalStorage.setJSON(EVALS_STORAGE_KEY, merged);
  ...
  async saveWeeklyEvaluation(evaluation: WeeklyEvaluation): Promise<void> {
    // 1. Save to local storage cache
    const allEvals = safeLocalStorage
      .getJSON<WeeklyEvaluation[]>(EVALS_STORAGE_KEY, [])
      .filter((e) => e.weeklyPlanId !== evaluation.weeklyPlanId);
    allEvals.push(evaluation);
    safeLocalStorage.setJSON(EVALS_STORAGE_KEY, allEvals);
    ...
    if (evaluation.userId && evaluation.userId !== 'guest') {
      ...
      if (!activeGoal) {
        throw new Error(
          '[PersonalLearningPlanService] Active goal not found in cache for evaluation save',
        );
      }
  ...
  async resetPlan(userId: string = 'guest'): Promise<void> {
    ...
    safeLocalStorage.removeItem(`${GOAL_STORAGE_PREFIX}${userId}`);

    const remainingPlans = safeLocalStorage
      .getJSON<WeeklyLearningPlan[]>(PLANS_STORAGE_KEY, [])
      .filter((p) => p.userId !== userId);
    safeLocalStorage.setJSON(PLANS_STORAGE_KEY, remainingPlans);

    const remainingEvals = safeLocalStorage
      .getJSON<WeeklyEvaluation[]>(EVALS_STORAGE_KEY, [])
      .filter((e) => e.userId !== userId);
    safeLocalStorage.setJSON(EVALS_STORAGE_KEY, remainingEvals);
  ...
  getCompletedLessonIds(userId: string = 'guest', language: SupportedLanguage): string[] {
    ...
    try {
      ...
      signals.forEach((sig: any) => {
        if (
          sig.type === 'completed_lesson' &&
          sig.lessonId &&
          (!language || sig.language === language)
        ) {

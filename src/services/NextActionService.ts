...
export const NextActionService = {
  ...
  getCandidateActions(state: UserLearningState): NextLearningAction[] {
    ...
    if (overdueCount > 0 || dueCount >= 5) {
      ...
      candidates.push({
        type: 'review_srs',
        title: isJa ? `${dueCount} ta fleshkartani takrorlang` : `Review ${dueCount} Flashcards`,
        description: isJa
        ...
        reason:
          overdueCount > 0
            ? isJa
              ? `${overdueCount} ta kartaning takrorlash muddati o'tib ketgan.`
              : `${overdueCount} cards are overdue for review.`
            : isJa
              ? `Bugun ${dueCount} ta so'zni takrorlash vaqti kelgan.`
              : `${dueCount} cards are scheduled for review today.`,
        ctaLabel: isJa ? '🧠 Takrorlashni Boshlash' : '🧠 Start Review',
        estimatedMinutes: estMinutes,
        priority: srsPriority,
        language: state.primaryLanguage,
        route: isJa ? '/study-mode?lang=ja' : '/study-mode',
        badgeIcon: '🧠',
        metadata: {
          dueCount,
          overdueCount,
    ...
    if (state.masteryProfile?.topWeaknesses && state.masteryProfile.topWeaknesses.length > 0) {
      ...
      const reasonText = topic
        ? isJa
          ? `So'nggi darslarda "${topic}" mavzusida xatoliklar qayd etilgan.`
          : `Recent mistakes detected in "${topic}".`
        : isJa
          ? `So'nggi darslarda bir nechta xatoliklar qayd etildi.`
          : `Multiple errors were detected in recent exercises.`;

      candidates.push({
        type: 'weakness_practice',
        title: isJa ? `Zaif ko'nikmalarni mustahkamlash` : `Strengthen Weak Skills`,
        description: isJa
    ...
    if (state.currentPosition) {
      ...
      const title = isFirst
        ? isJa
          ? `1-Dars: ${state.currentPosition.lessonTitle}`
          : `Lesson 1: ${state.currentPosition.lessonTitle}`
        : isJa
          ? `Keyingi dars: ${state.currentPosition.lessonTitle}`
          : `Next Lesson: ${state.currentPosition.lessonTitle}`;

      const reason = isFirst
        ? isJa
          ? `${state.targetLevel} darajasi bo'yicha boshlang'ich darsni boshlang.`
          : `Start the foundational lesson for ${state.targetLevel} level.`
        : isJa
          ? `O'quv rejangizdagi keyingi asosiy mavzuga o'tish vaqti keldi.`
          : `Time to advance to the next core lesson in your curriculum.`;

      const resolved = CurriculumLessonResolver.resolveLesson(
        state.currentPosition.lessonId,
        state.primaryLanguage,
      );

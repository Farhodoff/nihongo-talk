...
export function isAcousticEcho(userText: string, lastCoachText: string): boolean {
    ...
    if (normUser.length < 10 || normCoach.length < 10) return false;

    // 1. Direct Substring / Superstring match (requires >= 12 chars to avoid dropping greetings)
    const cleanUserNoSpace = normUser.replace(/\s+/g, '');
    const cleanCoachNoSpace = normCoach.replace(/\s+/g, '');

    if (cleanUserNoSpace.length >= 12 && (cleanCoachNoSpace.includes(cleanUserNoSpace) || cleanUserNoSpace.includes(cleanCoachNoSpace))) {
        return true;
    }
    ...
    if (/[\u3040-\u30ff\u3400-\u4dbf\u4e00-\u9fff]/.test(cleanUserNoSpace) && cleanUserNoSpace.length >= 10) {
        let matchingGrams = 0;
        let totalGrams = 0;

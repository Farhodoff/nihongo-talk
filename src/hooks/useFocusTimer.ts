...
export const useFocusTimer = (notificationsEnabled: boolean) => {
  const [focusState, setFocusState] = useState<FocusState>(() => {
    ...
    const parsed = safeLocalStorage.getJSON<any>('study_planner_focus_state', null);
    if (parsed) {
      try {
        ...
        return {
          ...parsed,
          isSessionCompleted: parsed.isSessionCompleted || false,
          bgSound: parsed.bgSound || 'none',
        ...
        new Notification('Vaqt tugadi!', {
          body:
            focusState.mode === 'focus'
              ? 'Ajoyib! Tanaffus vaqti.'
              : 'Tanaffus tugadi. Diqqatni jamlaymizmi?',
          icon: '/favicon.svg',
        });
      }
    }
  ...
  useEffect(() => {
    safeLocalStorage.setJSON('study_planner_focus_state', {
      ...focusState,
  ...
  const startTimer = () =>
    setFocusState((prev) => ({ ...prev, isActive: true, isSessionCompleted: false }));
  const pauseTimer = () => setFocusState((prev) => ({ ...prev, isActive: false }));
  const resetTimer = () => {
    const initial =
      focusState.mode === 'focus' ? 25 * 60 : focusState.mode === 'short_break' ? 5 * 60 : 15 * 60;
    setFocusState((prev) => ({
      ...prev,
      isActive: false,
      isSessionCompleted: false,
  ...
  const setFocusSubject = (id: string) =>
    setFocusState((prev) => ({ ...prev, selectedSubjectId: id }));
  const setFocusTask = (id: string | null) =>
    setFocusState((prev) => ({ ...prev, selectedTaskId: id }));

 ...
 */
export const UzbekistanFlag: React.FC<FlagProps> = ({
  ...
  <svg
    ...
    className={`inline-block shrink-0 overflow-hidden shadow-xs ${className}`}
    aria-hidden="true"
  >
    {/* Blue stripe */}
...
export const JapanFlag: React.FC<FlagProps> = ({ className = 'w-4 h-3 rounded-[2px]', size }) => (
  <svg
    ...
    height={size || undefined}
    className={`inline-block shrink-0 overflow-hidden border border-border/40 shadow-xs ${className}`}
    aria-hidden="true"
  >
...
/**
 * Clean SVG Vector Flag of the United Kingdom (English language)
 */
export const UKFlag: React.FC<FlagProps> = ({ className = 'w-4 h-3 rounded-[2px]', size }) => (
  <svg
    viewBox="0 0 60 30"
    width={size ? size * 1.6 : undefined}
    height={size || undefined}

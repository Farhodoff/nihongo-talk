...
describe('FuriganaText Component', () => {
  ...
  it('supports standard and full-width parenthesized furigana annotations', () => {
    mockUseStudyData.mockReturnValue({ settings: { showFurigana: true } });
    const { container } = render(<FuriganaText text="東京(とうきょう)と大阪（おおさか）" />);

    expect(screen.getByText('東京')).toBeInTheDocument();
    expect(screen.getByText('とうきょう')).toBeInTheDocument();
    expect(screen.getByText('大阪')).toBeInTheDocument();
    expect(screen.getByText('おおさか')).toBeInTheDocument();

    const rtElements = container.querySelectorAll('rt');
    expect(rtElements.length).toBe(2);
    expect(rtElements[0].textContent).toBe('とうきょう');
    expect(rtElements[1].textContent).toBe('おおさか');
  });
});

-- Migration: 20260910000000_strictly_single_superadmin_fsoyilov.sql
-- Description: Enforce strictly single superadmin (fsoyilov@gmail.com) across all SQL functions & tables

BEGIN;

-- 1. Demote any non-fsoyilov users to 'user' in profiles table
UPDATE public.profiles
SET role = 'user', updated_at = NOW()
WHERE LOWER(TRIM(email)) != 'fsoyilov@gmail.com'
  AND role IN ('admin', 'superadmin');

-- 2. Ensure fsoyilov@gmail.com is superadmin
UPDATE public.profiles
SET role = 'superadmin', updated_at = NOW()
WHERE LOWER(TRIM(email)) = 'fsoyilov@gmail.com';

-- 3. Upgrade is_admin() to strictly authorize ONLY fsoyilov@gmail.com
...
BEGIN
    ...
    IF _uid IS NOT NULL THEN
        ...
        IF _profile_role IN ('admin', 'superadmin') AND _profile_email = 'fsoyilov@gmail.com' THEN
            RETURN TRUE;
        END IF;
    END IF;
...
-- 4. Upgrade get_admin_all_users() function with single superadmin whitelist
...
BEGIN
    ...
    SELECT
        ...
        (CASE WHEN LOWER(TRIM(u.email)) = 'fsoyilov@gmail.com' THEN 'superadmin' ELSE COALESCE(p.role, 'user') END)::TEXT AS role,
        'unlimited'::TEXT AS tier,
        99999::INT AS ai_credits,
        u.created_at,
        u.last_sign_in_at

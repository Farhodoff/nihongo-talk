...
CREATE POLICY "Users and admins view study sessions" ON public.study_sessions FOR SELECT TO authenticated, anon USING (auth.uid() = user_id OR public.is_admin());
...
CREATE POLICY "Users and admins read speaking sessions" ON public.speaking_sessions FOR SELECT TO authenticated, anon USING (auth.uid() = user_id OR public.is_admin());
...
CREATE POLICY "Users and admins read speaking coach sessions" ON public.speaking_coach_sessions FOR SELECT TO authenticated, anon USING (auth.uid() = user_id OR public.is_admin());
...
CREATE POLICY "Users and admins view ai coach sessions" ON public.ai_coach_sessions FOR SELECT TO authenticated, anon USING (auth.uid() = user_id OR public.is_admin());

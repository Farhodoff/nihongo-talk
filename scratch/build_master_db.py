...
# Read extracted 35 N5 items string block
with open('/tmp/n5_clean_block.txt') as f:
    n5_block = f.read().strip().rstrip(',')

# Load 102 N4 items from /tmp/n4_parsed.json
...
n4_block = "// ==========================================\n"
n4_block += "// 🎌 JLPT N4 EXHAUSTIVE GRAMMAR (jlpt-n4-grammar-list.pdf - 102 items)\n"
n4_block += "// ==========================================\n"
n4_block += ",\n".join(formatted_n4)
...
header_types = """export interface JlptGrammarItem {
    id: string;
...
full_file_content = header_types + n5_block + ",\n\n    " + n4_block + ",\n" + n3_n2_n1_kanji

with open('src/data/jlptGrammarKanji.ts', 'w') as f:
    f.write(full_file_content)

print("Fixed syntax without trailing empty commas!")
